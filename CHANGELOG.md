...
- `apps/worker/src/jobs/image-to-pdf.ts` — new processor: for each input image, sniffs PNG/JPEG magic bytes, embeds via `pdf-lib`'s `embedPng`/`embedJpg`, adds a full-bleed page sized to that image's exact pixel dimensions, in upload order. Registered as a fifth BullMQ job name (`apps/worker/src/index.ts`, `apps/web/lib/queue.ts`).
- **Real bug found and fixed during manual end-to-end verification (not caught by unit tests):** `pdf-lib`'s JPEG embedder builds a `DataView` directly on the buffer's underlying `ArrayBuffer`, ignoring `Buffer.byteOffset`. `downloadFile()`'s real `Buffer.concat` return value is frequently pool-allocated at a nonzero offset for small files, corrupting pdf-lib's read of the JPEG SOI marker (`SOI not found in JPEG`) despite valid magic bytes and byte-identical storage round-trip — reproducible even in isolation. PNG embedding was unaffected (different decode path). Fixed by re-wrapping the buffer in `new Uint8Array(buffer)` before calling `embedPng`/`embedJpg`, which forces a byteOffset-0 copy; added a dedicated regression test reproducing the pool-allocation condition directly (confirmed it fails without the fix). See `wiki/lessons-learned.md`.
- `apps/web/app/api/image-to-pdf/jobs/route.ts` + `[jobId]/status/route.ts` + `[jobId]/download/route.ts` — new routes mirroring Merge's upload (1–10 files, PNG/JPEG magic-byte + size validation) and the ADR-008 ownership guard exactly.
- Added the `IMAGE_TO_PDF` entries to `apps/web/app/history/download-button.tsx`'s `JOB_TYPE_ROUTE_SLUGS` and `apps/web/app/history/page.tsx`'s `JOB_TYPE_LABELS` now rather than deferring to Session 037 as originally planned — adding the enum value made `tsc --noEmit` fail immediately (`Record<JobType, string>` exhaustiveness, by design per Sessions 033/034), so the two-line map entries were pulled forward to keep this session's own quality gates green. The actual `/image-to-pdf` frontend page and home page card remain Session 037 work.
- 9 new worker unit tests (`image-to-pdf.test.ts`, real `pdf-lib` run against Sharp-generated fixture PNG/JPEG buffers, only I/O boundaries mocked) + 26 new web route tests (`route.test.ts` × 3) + 2 new history regression tests (`download-button.test.tsx`, `page.test.tsx`) for the new lookup-table entries.
- Manually verified end-to-end against the real local stack (native Postgres/Redis/MinIO, worker + `next dev` restarted with `.env` loaded): submitted a mixed PNG (300×200) + JPEG (150×450) job via `curl`, confirmed `COMPLETED` status, downloaded the resulting PDF and verified 2 pages in upload order with each page's dimensions exactly matching its source image.
- `npm run typecheck` (0 errors), `npm run lint` (0 errors/warnings), `npm run test` (239 total — 213 web + 26 worker, all passing, no regressions).
- Covers AC-01–AC-14 and AC-18 of the Image to PDF spec; AC-15–AC-17 (frontend) and AC-19–AC-24 (quality/E2E) remain for Sessions 037–038.

...
import { CompressionLevel } from '@ilovepdf/shared'
...
const LEVELS: { value: CompressionLevel; label: string; description: string }[] = [
  { value: CompressionLevel.LOW, label: 'Low', description: 'Smaller reduction, higher image quality' },
  { value: CompressionLevel.RECOMMENDED, label: 'Recommended', description: 'Balanced size and quality' },
  { value: CompressionLevel.HIGH, label: 'High', description: 'Maximum reduction, lower image quality' },
]
...
export default function CompressPage() {
  ...
  const [level, setLevel] = useState<CompressionLevel>(CompressionLevel.RECOMMENDED)
  ...
  async function handleCompress() {
    ...
    try {
      const res = await fetch('/api/compress/jobs', { method: 'POST', body: formData })
      if (!res.ok) {
  ...
  const { data: jobStatus } = useQuery<JobStatusResponse>({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/compress/jobs/${jobId}/status`)
  ...
  useEffect(() => {
    ...
    if (jobStatus.status === 'COMPLETED') {
      ...
      setCompressError(jobStatus.errorMessage ?? 'An unknown error occurred.')
      setPhase('ERROR')
    }
  }, [jobStatus])
  ...
  if (phase === 'PROCESSING') {
    return (
      <main className="flex min-h-screen flex-col items-center justify-center gap-3 px-4">
        ...
        <p className="text-lg font-medium text-gray-700">Compressing your file…</p>
      </main>
    )
  }
  ...
  if (phase === 'DONE' && jobId) {
    const handleDownload = async () => {
      const res = await fetch(`/api/compress/jobs/${jobId}/download`)
      if (!res.ok) return
    ...
    const resetToIdle = () => {
      setFile(null)
      setFileError(null)
      setLevel(CompressionLevel.RECOMMENDED)
      setJobId(null)
    ...
    return (
      <main className="flex min-h-screen flex-col items-center justify-center gap-4 px-4 text-center">
        ...
        <h2 className="text-2xl font-bold text-gray-900">Your PDF has been compressed successfully</h2>
  ...
  if (phase === 'ERROR') {
    ...
    return (
      <main className="flex min-h-screen flex-col items-center justify-center gap-4 px-4 text-center">
        ...
        <h2 className="text-2xl font-bold text-gray-900">Compression failed</h2>
        <p className="text-sm text-gray-500">
          {compressError ?? 'Something went wrong while processing your file.'}
        </p>
        <button
          type="button"
          onClick={resetToIdle}
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 py-12">
      <h1 className="mb-2 text-3xl font-bold tracking-tight text-gray-900">Compress PDF</h1>
      <p className="mb-8 text-gray-500">
        Upload a PDF and choose a compression level to reduce its file size.
      </p>
      ...
      <p className="mb-1 block text-sm font-medium text-gray-700">Compression level</p>
      <div role="radiogroup" aria-label="Compression level" className="mb-4 grid grid-cols-3 gap-2">
        {LEVELS.map((option) => {

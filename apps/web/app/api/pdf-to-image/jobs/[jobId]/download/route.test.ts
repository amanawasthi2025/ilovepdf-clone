...
function buildRequest(jobId: string): NextRequest {
  return new NextRequest(`http://localhost/api/pdf-to-image/jobs/${jobId}/download`)
}

describe('GET /api/pdf-to-image/jobs/:jobId/download', () => {
  ...
  it('returns 200 with pre-signed url when job is COMPLETED', async () => {
    ...
    expect(mockGetPresignedUrl).toHaveBeenCalledWith(
      'outputs/abc.zip',
      expect.stringMatching(/^pdf-to-image-\d{4}-\d{2}-\d{2}\.zip$/),
    )
  })

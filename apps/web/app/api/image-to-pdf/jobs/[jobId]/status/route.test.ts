...
function buildRequest(jobId: string): NextRequest {
  return new NextRequest(`http://localhost/api/image-to-pdf/jobs/${jobId}/status`)
}

const FIXED_DATE = new Date('2026-06-30T10:00:00.000Z')

describe('GET /api/image-to-pdf/jobs/:jobId/status', () => {
  ...
  it('returns FAILED status with errorMessage populated', async () => {
    mockFindUnique.mockResolvedValue({
      ...
      createdAt: FIXED_DATE,
      updatedAt: FIXED_DATE,
      errorMessage: 'Input file is not a valid PNG or JPEG image',
    } as never)
    ...
    expect(body.errorMessage).toBe('Input file is not a valid PNG or JPEG image')
  })

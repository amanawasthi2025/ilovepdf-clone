...
function buildRequest(jobId: string): NextRequest {
  return new NextRequest(`http://localhost/api/compress/jobs/${jobId}/download`)
}

describe('GET /api/compress/jobs/:jobId/download', () => {
  ...
  it('returns 200 with pre-signed url when job is COMPLETED', async () => {
    ...
    expect(mockGetPresignedUrl).toHaveBeenCalledWith(
      'outputs/abc.pdf',
      expect.stringMatching(/^compressed-\d{4}-\d{2}-\d{2}\.pdf$/),
    )
  })

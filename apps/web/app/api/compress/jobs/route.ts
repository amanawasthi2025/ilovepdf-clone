...
const PDF_MAGIC = '%PDF'
const VALID_LEVELS = new Set<string>(Object.values(CompressionLevel))
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const levelInput = (formData.get('level') as string | null) ?? CompressionLevel.RECOMMENDED
  ...
  if (!VALID_LEVELS.has(levelInput)) {
    return errorResponse(
      'INVALID_COMPRESSION_LEVEL',
      `"${levelInput}" is not a valid compression level. Must be one of LOW, RECOMMENDED, HIGH.`,
      400,
    )
  }
  const level = levelInput as CompressionLevel
  ...
  try {
    ...
    if (err instanceof Error && err.message.includes('is encrypted')) {
      return errorResponse(
        'UNSUPPORTED_ENCRYPTED_PDF',
        `"${file.name}" is encrypted/password-protected and cannot be compressed.`,
        400,
      )
    }
  ...
  try {
    await documentProcessingQueue.add(
      'compress',
      { jobId: job.id, inputKey, level },
      { attempts: 3, backoff: { type: 'exponential', delay: 1000 } },
    )
  } catch (err) {
    logger.error({ err, jobId: job.id, correlationId }, 'Failed to enqueue compress job')
    await prisma.job.update({
      where: { id: job.id },
      data: { status: 'FAILED', errorMessage: 'Failed to enqueue job.' },
    }).catch(() => undefined)
    return errorResponse('INTERNAL_ERROR', 'Failed to enqueue compress job.', 500)
  }

  logger.info({ jobId: job.id, correlationId, level }, 'Compress job created')
